...
NATS 有一个内置的 **分布式** **持久性** 系统，称为 [JetStream](https://docs.nats.io/using-nats/developer/develop_jetstream)，它在基本的“核心 NATS”功能和服务质量之上支持新功能和更高的服务质量。
...
您可以使用 KV 存储桶来执行您期望从即时一致的键/值存储中获得的典型操作：

- put：将值与键关联
- get：检索与键关联的值
- delete：清除与键关联的任何值
- purge：清除与所有键关联的所有值
- create：仅当当前没有与该键关联的值时，才将该值与该键关联（即与 null 和 set 进行比较）
- update：比较并设置（又称比较和交换）键的值
- keys：获取所有key的副本（具有与之关联的值或操作）



**Watch**

**监控特定键的变化：** "watch" 功能允许客户端订阅某个特定键的更改通知。这就像在发布/订阅模式中订阅一个主题一样，但这里是针对键值的变化。一旦该键发生`put`（更新）或`delete`操作，所有订阅该键变化的“观察者”将实时接收到这些更新通知。这种能力对于需要即时响应数据变动的应用场景非常有用，比如实时数据分析、缓存同步或构建事件驱动的架构。

**Watch All**

**监控整个存储桶的所有变化：** "watch all" 功能则更进一步，它使客户端能够监视存储桶内所有键的更改。这意味着，无论哪个键发生了变化，客户端都会收到通知。这对于需要全局视图或维护数据一致性的服务来说是一个强大的工具，特别是在分布式系统中，帮助组件间保持数据同步和协调。

**History**

**检索键的历史记录：** "history" 功能允许用户获取与特定键相关的值及其删除操作的历史记录。默认情况下，大多数键值存储仅保存每个键的最新值，但启用历史记录功能后，可以追溯到过去的变更情况。这对于审计追踪、故障排查、数据分析或实现版本回溯等需求至关重要。需要注意，默认的历史记录深度是1，意味着除非特别配置，否则仅存储最新的值或操作。用户可以根据需要调整此设置以保留更多的历史版本。

综上所述，NATS KV存储通过提供这些高级功能，不仅支持基本的数据存储和检索，还促进了动态的、实时交互式系统的构建，并且增强了对数据演变过程的洞察力和可追溯性。这对于构建复杂、高响应性和高度协调的分布式应用系统尤为有利。
...
my_bucket > code revision: 1 created @ 26 Apr 24 03:12 UTC
...
![image-20240426111533727](./assets/image-20240426111533727.png)
